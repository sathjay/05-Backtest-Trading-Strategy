...
def plot_OBV(fig, df, row, column=1):
    ...
    fig.update_yaxes(title_text='Volume', row=row, col=column,secondary_y=True,)
...
app.layout = html.Div([
                     ...
                     html.H4('On Balance Volume Trading Strategy:'),
                    
                    html.P(['On-balance volume (OBV) is a simple accumulation-distribution tool that tallies up and down volume, \
                            creating a smooth indicator line to predict when major market moves might occur based on changes in relative trading volume.',
                            html.Br(), 
                            'On-balance volume provides a running total of an asset\'s trading volume and indicates whether this volume is flowing in \
                             or out of a given security or currency pair. The OBV is a cumulative total of volume (positive and negative).', 
                            html.Br() ,
                            'A Trader takes long position if the OBV line is higher than the OBV EMA line and short position if \
                                OBV line is lower than OBV EMA line',
                            html.Br() , 
                            ]),
...
@app.callback(
    ...
    [
     ...
     Input('OBV_EMA_period','value'),
     Input('backtest_days','value'),
    ])
def OBV_payoff(selected_stock,OBV_EMA_period,backtest_days):
    OBV_EMA_period = int(OBV_EMA_period)
    
    message = html.P("You have selected {} days as OBV time period".format(OBV_EMA_period))
    ...
    dff1['combined_signal'] = np.where(dff1['OBV']<dff1['OBV_EMA'],-1,dff1['combined_signal'])
