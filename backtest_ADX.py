...
def plot_ADX(fig, df, row, column=1):
    ...
    fig.update_yaxes(title_text='ADX and Directrional Movement Index', row=row, col=column)
...
app.layout = html.Div([
                    ...
                    html.H4('Average Directional Index (ADX) Strategy:'),
                    
                    html.P(['The Average Directional Index (ADX) is used to determine when the price is trending strongly.',
                            html.Br(), 
                            'ADX calculations are based on a moving average of price range expansion over a given period of time. The default setting is 14 days', 
                            html.Br() ,
                            'ADX is plotted as a single line with values ranging from a low of zero to a high of 100. ADX is non-directional it registers \
                            trend strength whether price is trending up or down. The indicator is usually plotted in the same window as the two directional \
...
@app.callback(
    ...
    [
     ...
     Input('ATR_period','value'),
     Input('backtest_days','value'),
    ])


def ADX_payoff(selected_stock,ATR_period,backtest_days):
    ATR_period = int(ATR_period)
    
    message = html.P("You have selected {} days as ADX time period".format(ATR_period))
    ...
    dff1['PlusDM'] = np.where((dff1['UpMove'] > dff1['DownMove']) & (dff1['UpMove'] > dff1['Zero']), dff1['UpMove'], 0)
    dff1['MinusDM'] = np.where((dff1['UpMove'] < dff1['DownMove']) & (dff1['DownMove'] > dff1['Zero']), dff1['DownMove'], 0)

    dff1['plusDI'] = 100 * (dff1['PlusDM']/dff1['ATR']).ewm(span=ATR_period,min_periods=0,adjust=True,ignore_na=False).mean()
    dff1['minusDI'] = 100 * (dff1['MinusDM']/dff1['ATR']).ewm(span=ATR_period,min_periods=0,adjust=True,ignore_na=False).mean()

    dff1['ADX'] = 100 * (abs((dff1['plusDI'] - dff1['minusDI'])/(dff1['plusDI'] + dff1['minusDI']))).ewm(span=ATR_period,min_periods=0,adjust=True,ignore_na=False).mean()
    ...
    dff1['combined_signal'] = np.where(dff1['plusDI']<dff1['minusDI'],-1,dff1['combined_signal'])
