...
app.layout = html.Div([
                     ...
                     html.H4('Simple Moving Average (SMA) Crossover Strategy:'),
                     
                    html.P('For stock selected from the below drop down, users can check whether SMA trading strategy \
                    is has been profitable or not in the past. In SMA Crossover strategy, trader takes long position when \
                    the Fast moving average (short duration) price is above the Slow moving (long duration) average price and \
                    takes short position when the Fast moving average (short duration) price is below the Slow moving \
                    (long duration) average price'),
                    ...
                    html.Hr(),
                    dcc.Graph(id='entry_and_exit_SMA',figure = blank_fig(),
                              style={'width': '90%', 'height': '80vh'}),
                    html.Hr(), 
                    dcc.Graph(id='pnl_chart_SMA',figure = blank_fig(),
                              style={'width': '90%', 'height': '80vh'}),
                    html.Hr(), 
...
def caluculate_moving_average_payoff(selected_stock,FMA,SMA,backtest_days):
    ...
    if  SMA <= FMA:
        ...
        dff1['combined_signal'] = np.where(dff1['Fast_MA']>dff1['Slow_MA'],1,0)
        dff1['combined_signal'] = np.where(dff1['Fast_MA']<dff1['Slow_MA'],-1,dff1['combined_signal'])
