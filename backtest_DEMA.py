...
app.layout = html.Div([
                    ...
                    html.H4('Double Exponential Moving Average (DEMA) Crossover Strategy:'),
                     
                    html.P('For stock selected from the below drop down, users can check whether DEMA trading strategy \
                    has been profitable or not in the past. In DEMA Crossover strategy, trader takes long position when \
                    the short duration(Fast) DEMA price is above the long duration(Slow) DEMA price and takes short position when \
                    the short duration(Fast) DEMA price is lower than long duration(Slow) DEMA price'),
...
def caluculate_double_expo_moving_average_payoff(selected_stock,FMA,SMA,backtest_days):
    ...
    if  SMA <= FMA:
        ...
        dff['Fast_DEMA'] = (2*dff['Fast_EMA']) - dff['Fast_EMA'].ewm(span = FMA,adjust = False).mean()
        dff['Slow_DEMA'] = (2*dff['Slow_EMA']) - dff['Slow_EMA'].ewm(span = SMA,adjust = False).mean()
        ...
        dff1['combined_signal'] = np.where(dff1['Fast_DEMA']>dff1['Slow_DEMA'],1,0)
        dff1['combined_signal'] = np.where(dff1['Fast_DEMA']<dff1['Slow_DEMA'],-1,dff1['combined_signal'])
        ...
        dff1['long_signal'] = np.where(dff1['Fast_DEMA']>dff1['Slow_DEMA'],1,0)
