...
app.layout = html.Div([
                     ...
                     html.H4('Money Flow Index Strategy:'),
...
@app.callback(
    ...
    [
     ...
     Input('MFI_Period','value'),
     Input('upper_MFI','value'),
     Input('lower_MFI','value'),
     Input('backtest_days','value'),
    ])
def MFI_update(selected_stock,MFI_Period,upper_MFI,lower_MFI,backtest_days):
    ...
    dff['MFR'] = dff['Positive_flow'].rolling(window = MFI_Period,center = False).sum()/dff['Negative_flow'].rolling(window = MFI_Period,center = False).sum()
    ...
    dff1['Position'] = np.where(dff1['Money_Flow_Index'] > upper_MFI, -1, np.nan)
    dff1['Position'] = np.where(dff1['Money_Flow_Index'] < lower_MFI, 1, dff1['Position'])
